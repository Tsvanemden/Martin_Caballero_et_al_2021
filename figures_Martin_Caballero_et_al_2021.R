...
source("functions_caballero_et_al_2021.R")
...
res.all <- Reduce(function(x, y) merge(x, y, all=TRUE, by = "genes"), list(res.lem2, res.rrp6, res.red1, res.erh1, res.air1, res.ccr4, res.clr4, res.man1))
...
# Save figure 1
ggsave("figures/fig1.pdf", fig1, width = 8.5/2.54, height = 20/2.54, device = pdf, useDingbats=FALSE)
...
# Save figure S1
ggsave("figures/figS1.pdf", figS1, width = 17/2.54, height = 10/2.54, device = pdf, useDingbats=FALSE)
...
res.fig2b <- res.fig2b[,1:6]

# Do the Kmeans clustering
k <- kmeans(na.omit(res.fig2b), centers = 10, iter.max = 1000, nstart = 1)
...
dfc$average <- rowMeans(dfc[,1:6])
test <- aggregate(dfc[, 9], list(dfc$cluster), mean)
test$Group.1[test$Group.1 == 1:8] <- letters[seq( from = 1, to = 8 )]
test <- test[order(test$x),]
test$iddsort <- 1:8

# Reorder clusters
dfc$cluster <- mapvalues(dfc$cluster, from = 1:8, to = letters[seq( from = 1, to = 8 )])
...
dfm$variable <- factor(dfm$variable,levels = c("lem2", "rrp6", "red1","air1" ,"erh1", "ccr4", "cluster"))
